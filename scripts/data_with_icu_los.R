...
library(Hmisc)
#Read Data
data <- read_csv('data/data_with_icu_los.csv') %>%
  mutate(
    bmi  = weight / (height/100)^2,
    crea = as.numeric(gsub(",", ".", gsub("\\.", "", crea))),
    pre_crea = as.numeric(gsub(",", ".", gsub("\\.", "", pre_crea)))
  )
#Setting Labels
...
#label(data$pat_icu_date_i)="ICU Admit Date"
#label(data$pat_icu_time_i)="ICU Admit Time"
#label(data$pat_icu_date)="ICU Admit Date"
#label(data$pat_icu_time)="ICU Admit Time"
#Setting Units
...
data$redcap_repeat_instrument.factor = factor(data$redcap_repeat_instrument,levels=c("eclsmode","eclsequipment","pump","hemodynamics_ventilation_medication","labor","complications","mode_and_equipment_impella","impellaassessment_and_complications","impella_follow_up","surgery","coronary_angiography","echocardiography","infections","additional_mcs"))
...
levels(data$redcap_repeat_instrument.factor)=c("ECLS-Mode","ECLS-Equipment","Pump","Hemodynamics, Ventilation, Medication","Labor","Complications","Mode and Equipment Impella","Impella-Assessment and Complications","Impella - Follow up","Surgery","Coronary Angiography","Echocardiography","Infections","Additional MCS")
