...
weighted_subjects <- data %>%
  weightit(formula = group ~ age + sex + ns(bmi) + ns(lactate) + ns(vis_score), estimand = "ATT", method = "glm")
...
plot(weighted_subjects, type = "jitter", interactive = FALSE)
dev.off()

# evaluate match of densities after matching
png("figs/propensity_scores_densities.png",
    width = 25, height = 25, units = "cm", res = 300)
plot(weighted_subjects, type = "density", interactive = FALSE,
...
png("figs/propensity_scores_qq.png",
    width = 25, height = 25, units = "cm", res = 300)
plot(weighted_subjects, type = "qq", interactive = FALSE,
